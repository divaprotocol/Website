...
You can check your reward [here](#https://www.divaprotocol.io/). Overall, a total of 1.12m DIVA tokens has been allocated to more than 13'600 users, 
12% more than originally planned due to rounding up the rewards.

Tokens can be claimed once the token is launched which is expected to coincide with the mainnet launch of the protocol in Q1 2023.

For those that are interested in the details of the verification, read on.
...
verification process. We decided to go with the Double Counter Discord bot as this was considered the most simple and efficient solution which didn't require users to share 
...
Users that are eligible for the reward:
* Users that registered their wallet with the #testnet bot during the testnet phase
* Users that got verified by the Double Counter bot on first attempt
* User that were blocked on first attempt because of using a VPN but got verified by the Double Counter bot on second attempt
* Holders of an 888Whales NFT
* Quiz winners
