...
The participation was overwhelming, more than we have ever imagined. Before we dive into the wrap-up and the outlook, we want to say a big THANK YOU to everyone
who participated in our testnet. The huge participation was a real stress test for our app and
helped us to iron out a lot of bugs and issues that we wouldn't have spotted otherwise.

Here some key statistics for the records:

- Total pools created: >170k
- Unique addresses participated: >26k
- Number users that completed all tasks: >13k
- Discord users: >33k (we started off with 800 members)

## DIVA token reward

The most important topic for testnet participants is the DIVA token reward that they receive for completing the testnet tasks.

Before we mention the reward, let's briefly recap the methodology that we have outlined in our
[testnet announcement article](#https://www.divaprotocol.io/posts/diva-testnet-announcement) back in June:
...
Every participant would have received 4500 DIVA tokens if total points collected by users summed up to 1 million or less. This is equivalent to
222 participants completing all the testnet tasks. With 800 discord members before the testnet launch and without any major marketing plans, we deemed this to be
an ambitious target back then. We were proven wrong, to our positive surprise.

The implication of this the high testnet participation is that each participant's reward got diluted down to 100 DIVA tokens which is equivalent to $10 assuming a 10m token valuation.

**Don't be disappointed, read on.**
